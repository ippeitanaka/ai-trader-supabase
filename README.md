...
- `STANDARD_M15`: 従来の標準モード。M15で判断し、H1を上位足監査に使う
- `SCALP_M5`: 短期モード。M5で判断し、M15を上位足監査に使う。実行勝率ゲート62%、RR 1.1、ATRリスク幅 1.4、最大保有30分、決済後クールダウン5分が既定
- 短期モードの実行勝率は「TP先着、または30分の時間決済時にコスト控除後プラス」の確率。銘柄別手動設定がある場合のみダッシュボードの値を優先する
- MT5で表示しているチャート時間足には依存せず、EAが運用モードに応じた時間足データを内部取得する
- 両モードを同時運用する場合はEAを別チャートへ1つずつ設置する。短期モードは標準モードと異なるMagic番号を自動使用する
- ネッティング口座では同一銘柄の両モードを同時運用しない。ヘッジ口座では独立して追跡できる
