...
const PRIMARY_OPENAI_MODEL = Deno.env.get("AI_TRADER_EXECUTION_MODEL") || "gpt-5-mini";
const FALLBACK_OPENAI_MODEL = Deno.env.get("OPENAI_MODEL") || "gpt-4.1-mini";
...
async function fetchCalibrationRows(
  ...
  const queryLimit = includeVirtual ? Math.max(limit, 1000) : limit;
...
async function getExecutionModelHealth(timeframe: string): Promise<ExecutionModelHealth> {
  ...
  const { data, error } = await supabase
    .from("ai_signals")
    .select("created_at,symbol,dir,win_prob_calibrated,win_prob_raw,win_prob,actual_result")
    .gte("created_at", since)
  ...
  if (error) {
    console.warn(`[model-health] fetch error: ${error.message}`);
    return { weight: 0, episodes: 0, separation: null };
  }
  ...
  const episodes = collapseTimedEpisodes(rows)
    .map((row) => ({
      probability: typeof row.win_prob_calibrated === "number"
        ? row.win_prob_calibrated
        : typeof row.win_prob_raw === "number"
        ? row.win_prob_raw
        : row.win_prob,
      outcome: row.actual_result === "WIN" ? 1 : 0,
    }))
    .filter((row): row is { probability: number; outcome: number } =>
      typeof row.probability === "number" && Number.isFinite(row.probability)
    );

  const wins = episodes.filter((row) => row.outcome === 1);
  const losses = episodes.filter((row) => row.outcome === 0);
  const averageProbability = (items: typeof episodes) =>
    items.length > 0 ? items.reduce((sum, row) => sum + row.probability, 0) / items.length : null;
  const winAverage = averageProbability(wins);
  const lossAverage = averageProbability(losses);
  const separation = winAverage !== null && lossAverage !== null ? winAverage - lossAverage : null;
  const minimumEpisodes = 20;
  const sampleFactor = Math.max(0, Math.min(1, (episodes.length - minimumEpisodes) / 40));
  const rankStrength = separation !== null ? Math.max(0, Math.min(1, (separation - 0.01) / 0.09)) : 0;
  const health = {
    weight: round3(sampleFactor * rankStrength),
    episodes: episodes.length,
    separation: separation === null ? null : round3(separation),
  };
...
function applyChartQualityGuard(tradeReq: TradeRequest, response: TradeResponse): TradeResponse {
  ...
  const expectedValueR = typeof response.expected_value_r === "number"
  ...
  if (canOverride) {
    ...
    return {
      ...response,
      plan_alignment: "opportunity_override",
      decision_summary: appendReasonText(response.decision_summary, `chart_override=${softReasons.join("+")}`),
      reasoning: appendReasonText(response.reasoning, overrideNote),
      method_reason: appendReasonText(response.method_reason, overrideNote),
    };
  }
...
async function applyDailyPlanGuard(tradeReq: TradeRequest, response: TradeResponse): Promise<TradeResponse> {
  ...
  if (typeof expiresAt === "string") {
    ...
    if (Number.isFinite(expiresMs) && Date.now() > expiresMs) hardReasons.push("daily_plan_expired");
  }
  ...
  if (item && actionDir !== 0) {
    ...
    if (allowed === "none" || (allowed !== "both" && actual !== allowed)) {
      hardReasons.push("daily_plan_direction_mismatch");
    ...
    if (typeof item.max_cost_r === "number" && costR !== null && costR > item.max_cost_r) {
      softReasons.push("daily_plan_cost_too_high");
      alignment = "plan_gate_miss";
    }
  ...
  const expectedValueR = typeof response.expected_value_r === "number"
    ...
    : computeExpectedValueR(response.win_prob, response.reward_rr ?? 1.5, response.cost_r ?? 0);
  const costR = typeof response.cost_r === "number" ? response.cost_r : 0;
  const minOverrideEv = softReasons.includes("daily_plan_htf_conflict")
    ? 0.18
  ...
  if (canOverride) {
    ...
    return {
      ...withDiagnostics,
      plan_alignment: "opportunity_override",
      decision_summary: appendReasonText(withDiagnostics.decision_summary, `plan_override=${softReasons.join("+")}`),
      reasoning: appendReasonText(withDiagnostics.reasoning, overrideNote),
      method_reason: appendReasonText(withDiagnostics.method_reason, overrideNote),
    };
  }
...
function applyExecutionGuards(_tradeReq: TradeRequest, response: TradeResponse): TradeResponse {
  // Pair-specific historical vetoes become stale quickly. Daily selection,
  // calibrated probability, live costs, and chart context now own this decision.
  return response;
}
...
function getMinEvR(): number {
  const v = Number(Deno.env.get("AI_TRADER_OPPORTUNITY_MIN_EV_R") ?? 0.05);
...
• 取引しないことを安全性と誤認せず、正の期待値がある候補を正しく評価してください`;
  ...
  try {
    ...
    console.log(`[AI] Prediction model: ${prediction.model}`);
  ...
  if (req.method === "GET") {
    ...
    return new Response(
      JSON.stringify({ 
        ...
        openai_fallback_model: FALLBACK_OPENAI_MODEL,
