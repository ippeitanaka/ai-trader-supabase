-- Migration: Add regime/strategy classification fields
--
-- Purpose:
-- - Persist the market regime (trend/range/uncertain) and chosen strategy (trend_follow/mean_revert/none)
--   for later analysis and debugging.

-- ai_signals (learning / post-trade analysis)
ALTER TABLE public.ai_signals
  ADD COLUMN IF NOT EXISTS regime TEXT NULL,
  ADD COLUMN IF NOT EXISTS strategy TEXT NULL,
  ADD COLUMN IF NOT EXISTS regime_confidence TEXT NULL;

CREATE INDEX IF NOT EXISTS idx_ai_signals_regime
  ON public.ai_signals (regime)
...
CREATE INDEX IF NOT EXISTS idx_ea_log_regime
  ON public."ea-log" (regime)
  WHERE regime IS NOT NULL;

-- Update monitor view to show regime/strategy when available
-- NOTE: CREATE OR REPLACE VIEW cannot change existing view column names/order.
-- Drop first to keep this migration robust across prior view versions.
...
COMMENT ON VIEW public.ea_log_monitor IS 'EAログ（方向/勝率/AI推奨・BUY/SELL比較・相場状態/戦略）を見やすく表示（AI推奨はsuggested_dirから補完）';
